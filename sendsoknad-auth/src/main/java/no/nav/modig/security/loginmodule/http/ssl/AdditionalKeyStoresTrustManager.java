...
public class AdditionalKeyStoresTrustManager implements X509TrustManager {
    ...
    public void checkServerTrusted(X509Certificate[] chain, String authType) throws CertificateException {
        ArrayList<Exception> exceptions = new ArrayList<Exception>(x509TrustManagers.size());
        ...
        for (X509TrustManager tm : x509TrustManagers) {
            try {
                ...
                exceptions.add(e);
                if (isConfigured) {
                    log.debug("Caught exception in checkServerTrusted() from default X509TrustManager (with default keystore). Exception: " + e);
                } else {
                    log.debug("Caught exception in checkServerTrusted() from configured X509TrustManager (with additional keystore). Exception: " + e);
                }
